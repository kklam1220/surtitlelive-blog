...
function buildPrompt({ locale, source }) {
  ...
  return [
    ...
    "- Do not alter URLs, image paths, code blocks, inline code spans, or frontmatter key names.",
...
function getLocalizedArticleInstructions(locale, source) {
  ...
  if (source.slug === "13-quick-qlab-subtitles-from-excel-txt") {
    return [
      "- Preserve the exact QLab cue-name tokens `CLEAR PREVIOUS`, `DISPLAY`, and `CLEAR LAST SUBTITLE` wherever they appear. They are literal names generated by the companion AppleScript, not prose to translate. The language code after `DISPLAY` may match the target locale.",
      "- Preserve all other inline-code identifiers exactly. Translate only the surrounding explanation.",
      "- Keep the workflow explicit: the importer creates top-level Timeline Groups in the active QLab cue list and does not wrap them in a temporary import Group.",
    ];
  }
...
async function translateWithGemini({ apiKey, baseUrl, model, locale, source }) {
  ...
  const body = await callGeminiText({
    ...
    text: [
      ...
      "Preserve headings, lists, links, code blocks, inline code spans, HTML tags, and all URLs/image paths.",
